 ...
 *
 * @param currentPage - 当前页码（从 1 开始） / Current page number (1-based)
 * @param totalPages - 总页数 / Total number of pages
 * @returns 页码和省略号字符串的数组 / Array of page numbers and ellipsis strings
 *
 * @example
...
export function getPageNumbers(currentPage: number, totalPages: number) {
  /** 最大可见页码按钮数 / Maximum number of page buttons to show */
  const maxVisiblePages = 5
  const rangeWithDots = []

  if (totalPages <= maxVisiblePages) {
    /** 总页数 <=5 时显示全部 / Show all pages when total is 5 or less */
    for (let i = 1; i <= totalPages; i++) {
      rangeWithDots.push(i)
    }
  } else {
    /** 始终显示第一页 / Always show the first page */
    rangeWithDots.push(1)

    if (currentPage <= 3) {
      /** 靠近开头: [1] [2] [3] [4] ... [10] / Near the beginning */
      for (let i = 2; i <= 4; i++) {
        rangeWithDots.push(i)
      }
      rangeWithDots.push('...', totalPages)
    } else if (currentPage >= totalPages - 2) {
      /** 靠近末尾: [1] ... [7] [8] [9] [10] / Near the end */
      rangeWithDots.push('...')
      for (let i = totalPages - 3; i <= totalPages; i++) {
        rangeWithDots.push(i)
      }
    } else {
      /** 在中间: [1] ... [4] [5] [6] ... [10] / In the middle */
      rangeWithDots.push('...')
